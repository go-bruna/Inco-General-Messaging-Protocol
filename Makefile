...
clone_fhevm_tfhe_cli: $(WORKDIR)/
	...
	cd $(WORKDIR) && git clone https://github.com/zama-ai/fhevm-tfhe-cli.git
...
clone_fhevm_solidity: $(WORKDIR)/
	...
	cd $(WORKDIR) && git clone https://github.com/zama-ai/fhevm-solidity.git
...
clone_tfhe_rs: $(WORKDIR)/
	...
	cd $(WORKDIR) && git clone https://github.com/zama-ai/tfhe-rs.git
...
clone_fhevm_decryptions_db: $(WORKDIR)/
	...
	cd $(WORKDIR) && git clone https://github.com/zama-ai/fhevm-decryptions-db.git
...
clone_go_ethereum: $(WORKDIR)/
	...
	@if [ -d "$(WORKDIR)/go-ethereum" ] && [ "$(ls -A $(WORKDIR)/go-ethereum)" ]; then \
		...
		cd $(WORKDIR) && git clone https://github.com/zama-ai/go-ethereum.git; \
...
clone_ethermint: $(WORKDIR)/
	...
	echo "$(WORKDIR)/ethermint already exists and is not empty. Skipping git clone."; \
		...
		cd $(WORKDIR) && git clone https://github.com/zama-ai/ethermint.git; \
