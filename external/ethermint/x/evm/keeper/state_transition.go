...
func (k *Keeper) NewEVM(
	...
	ctx.EventManager().EmitEvents(
		sdk.Events{
			sdk.NewEvent(
				"EVM block gas limist",
				sdk.NewAttribute("error content", fmt.Sprintf("%d", ethermint.BlockGasLimit(ctx))),
			),
		},
	)
...
func (k *Keeper) ApplyMessageWithConfig(ctx sdk.Context, msg core.Message, tracer vm.EVMLogger, commit bool, cfg *types.EVMConfig, txConfig statedb.TxConfig) (*types.MsgEthereumTxResponse, error) {
	...
	k.Logger(ctx).Info("ApplyMessageWithConfig")

	// return error if contract creation or call are disabled through governance
	if !cfg.Params.EnableCreate && msg.To() == nil {
		k.Logger(ctx).Error("failed to create new contract")

		return nil, sdkerrors.Wrap(types.ErrCreateDisabled, "failed to create new contract param disabled")
	} else if !cfg.Params.EnableCall && msg.To() != nil {
		k.Logger(ctx).Error("failed to call contract param disabled")
	...
	ctx.EventManager().EmitEvents(
		sdk.Events{
			sdk.NewEvent(
				"EVM",
				sdk.NewAttribute("NewEVM", "NewEVM"),
			),
		},
	)

	k.Logger(ctx).Info("After NewEVM")
	...
	k.Logger(ctx).Info("before GetEthIntrinsicGas")

	intrinsicGas, err := k.GetEthIntrinsicGas(ctx, msg, cfg.ChainConfig, contractCreation)
	if err != nil {
		k.Logger(ctx).Error("before GetEthIntrinsicGas", sdkerrors.Wrap(err, "intrinsic gas failed"))

		// should have already been checked on Ante Handler
		return nil, sdkerrors.Wrap(err, "intrinsic gas failed")
	}

	k.Logger(ctx).Info("after GetEthIntrinsicGas")

	// Should check again even if it is checked on Ante Handler, because eth_call don't go through Ante Handler.
	if leftoverGas < intrinsicGas {
		k.Logger(ctx).Error("ErrIntrinsicGas apply message failed", sdkerrors.Wrap(core.ErrIntrinsicGas, "apply message"))
	...
	k.Logger(ctx).Info("Before Call EVM")
	...
	if contractCreation {
		...
		// ret, leftoverGas, vmErr = evm.Call(sender, *msg.To(), msg.Data(), (uint64)(5000000000), msg.Value())
	}

	k.Logger(ctx).Info("After Call EVM")
	...
	if msg.Gas() < leftoverGas {
		k.Logger(ctx).Error("ErrGasOverflow apply message failed", sdkerrors.Wrap(types.ErrGasOverflow, "apply message"))
	...
	k.Logger(ctx).Info("Before Commit to StateDB")

	// The dirty states in `StateDB` is either committed or discarded after return
	if commit {
		if err := stateDB.Commit(); err != nil {
			k.Logger(ctx).Error("failed to commit stateDB", sdkerrors.Wrap(err, "failed to commit stateDB"))
	...
	if msg.Gas() < leftoverGas {
		k.Logger(ctx).Error(fmt.Sprintf("message gas limit < leftover gas (%d < %d)", msg.Gas(), leftoverGas))
	...
	for _, log := range logs {
		k.Logger(ctx).Info("Logs: " + string(log.Data))
	}
...
func (k *Keeper) ApplyMessage(ctx sdk.Context, msg core.Message, tracer vm.EVMLogger, commit bool) (*types.MsgEthereumTxResponse, error) {
	...
	k.Logger(ctx).Info("Before Apply Message With Config")
