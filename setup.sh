...
#TRACE="--trace"
TRACE=""
HOME_EVMOSD="/home/ubuntu/.evmosd"

EVMOSD="/home/ubuntu/go/bin/evmosd"
...
cat $HOME_EVMOSD/config/genesis.json | jq '.consensus_params["block"]["max_gas"]="1000000000"' > $HOME_EVMOSD/config/tmp_genesis.json && mv $HOME_EVMOSD/config/tmp_genesis.json $HOME_EVMOSD/config/genesis.json
...
touch $HOME_EVMOSD/privkey
$EVMOSD keys unsafe-export-eth-key mykey1 --keyring-backend test > $HOME_EVMOSD/privkey
touch $HOME_EVMOSD/node_id
$EVMOSD tendermint show-node-id > $HOME_EVMOSD/node_id
